...
gears  = "*"
n = 0
for n_line, line in enumerate(input):
    for n_elem, elem in enumerate(line):
        if elem == gears:
            found = []
            print(elem, n_line, n_elem)
            for i in range(3):
                for j in range(3):
                    print(n_line-1+i, n_elem-1+j)
                    if input[n_line-1+i][n_elem-1+j] in numbers:
                        g, d = 0, 0
                        while input[n_line-1+i][n_elem-1+j-g] in numbers:
                            g += 1
                        g -= 1
                        while n_elem-1+j+d < len(line) and input[n_line-1+i][n_elem-1+j+d] in numbers:
                            d += 1
                        if int(input[n_line-1+i][n_elem-1+j-g:n_elem-1+j+d]) not in found:
                            found.append(int(input[n_line-1+i][n_elem-1+j-g:n_elem-1+j+d]))
            if len(found) == 2:
                n += found[0]*found[1]
print(n)