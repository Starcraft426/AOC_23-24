...
input2 = """.F7FFJ7L-7.F-7.F7|F-L-7F7F|7.F7F7FJ.FLFF-77.FFF7-|FL-7LF-7F7FF--77.-777F-LJFFL7FJ.|-L7-FF-FJ77F7F7F-7F.|F7JF-L-J--|-F7|F-LF--77..J..F---.F7-
...
FJ-L--7J--J-LL--LL77-LL.L7J.JJJJL-|J..J----7L-JL|-J-|J-LJF-|.L-LJ..|JL-JLLLJJJ--7LJL-J.|L--FFJ.|JL-J.J-L|---F--|L|..7.L|.F|L-J--JJ.-.FJLL7JJ""".split("\n")
...
changes = {"-":{"gauche":{True:None, False:None}, "haut":{True:None, False:None}},
           "|":{"gauche":{True:None, False:None}, "haut":{True:None, False:None}},
           "F":{"gauche":{True:True, False:False}, "haut":{True:True, False:False}},
           "7":{"gauche":{True:False, False:True}, "haut":{True:False, False:True}},
           "J":{"gauche":{True:True, False:False}, "haut":{True:True, False:False}},
           "L":{"gauche":{True:False, False:True}, "haut":{True:False, False:True}}
          }

input2 = [list(elem) for elem in input2]
input = deepcopy(input2)
x, y = find_start(input)
n_etapes = 0

pos1, pos2 = (x, y), (x, y)
pos1_tile, pos2_tile = "S", "S"
haut = True
gauche = True
not_changing = None
input[x][y] = 0
while pos1 != pos2 or n_etapes == 0:
    n_etapes += 1
    if pos1[0]-1 > -1 and input[pos1[0]-1][pos1[1]] in ["|", "7", "F"] and pos1_tile in ["|", "L", "J", "S"]:
        pos1 = (pos1[0]-1, pos1[1])
        not_changing = 1
    elif pos1[0]+1 < len(input) and input[pos1[0]+1][pos1[1]] in ["|", "L", "J"] and pos1_tile in ["|", "7", "F", "S"]:
        pos1 = (pos1[0]+1, pos1[1])
        not_changing = 1
    elif pos1[1]-1 > -1 and input[pos1[0]][pos1[1]-1] in ["-", "L", "F"] and pos1_tile in ["-", "7", "J", "S"]:
        pos1 = (pos1[0], pos1[1]-1)
        not_changing = 3
    elif pos1[1]+1 < len(input[0]) and input[pos1[0]][pos1[1]+1] in ["-", "7", "J"] and pos1_tile in ["-", "L", "F", "S"]:
        pos1 = (pos1[0], pos1[1]+1)
        not_changing = 3
    else:
        break
    pos1_tile = input[pos1[0]][pos1[1]]
    input[pos1[0]][pos1[1]] = n_etapes

    if pos2[0]-1 > -1 and input[pos2[0]-1][pos2[1]] in ["|", "7", "F"] and pos2_tile in ["|", "L", "J", "S"]:
        pos2 = (pos2[0]-1, pos2[1])
    elif pos2[0]+1 < len(input) and input[pos2[0]+1][pos2[1]] in ["|", "L", "J"] and pos2_tile in ["|", "7", "F", "S"]:
        pos2 = (pos2[0]+1, pos2[1])
    elif pos2[1]-1 > -1 and input[pos2[0]][pos2[1]-1] in ["-", "L", "F"] and pos2_tile in ["-", "7", "J", "S"]:
        pos2 = (pos2[0], pos2[1]-1)
    elif pos2[1]+1 < len(input[0]) and input[pos2[0]][pos2[1]+1] in ["-", "7", "J"] and pos2_tile in ["-", "L", "F", "S"]:
        pos2 = (pos2[0], pos2[1]+1)
    else:
        break
    pos2_tile = input[pos2[0]][pos2[1]]
    input[pos2[0]][pos2[1]] = n_etapes

for i, line in enumerate(input2):
    for j, elem in enumerate(line):
        if type(input[i][j]) is str:
            input2[i][j] = " "
        elif input[i][j] == -1:
            input2[i][j] = "*"

x, y = find_start(input2)
input2[x][y] = "-"
found = False
for i, line in enumerate(input2):
    for j, elem in enumerate(line):
        if elem == "F" and not found:
            x, y = i, j
            found = True
            break
    if found:
        break
input3 = deepcopy(input2)
pos1, pos2 = (x, y), (x, y)
pos1_tile, pos2_tile = "F", "F"
haut = True
gauche = True
not_changing = None
n_etapes = 0
input2[x][y] = 0
while pos1 != (x, y) or n_etapes == 0:
    try:
        if haut:
            if input2[pos1[0]-1][pos1[1]] == " ":
                input2[pos1[0]-1][pos1[1]] = "*"
        else:
            if input2[pos1[0] + 1][pos1[1]] == " ":
                input2[pos1[0]+1][pos1[1]] = "*"
    except IndexError:
    ...
    try:
        if gauche:
            if input2[pos1[0]][pos1[1]-1] == " ":
                input2[pos1[0]][pos1[1]-1] = "*"
        else:
            if input2[pos1[0]][pos1[1]+1] == " ":
                input2[pos1[0]][pos1[1]+1] = "*"
    except IndexError:
        pass
    n_etapes += 1
    if pos1[0]-1 > -1 and input2[pos1[0]-1][pos1[1]] in ["|", "7", "F"] and pos1_tile in ["|", "L", "J"]:
        pos1 = (pos1[0]-1, pos1[1])
        not_changing = 1
    elif pos1[0]+1 < len(input2) and input2[pos1[0]+1][pos1[1]] in ["|", "L", "J"] and pos1_tile in ["|", "7", "F"]:
        pos1 = (pos1[0]+1, pos1[1])
        not_changing = 1
    elif pos1[1]-1 > -1 and input2[pos1[0]][pos1[1]-1] in ["-", "L", "F"] and pos1_tile in ["-", "7", "J"]:
        pos1 = (pos1[0], pos1[1]-1)
        not_changing = 3
    elif pos1[1]+1 < len(input2[0]) and input2[pos1[0]][pos1[1]+1] in ["-", "7", "J"] and pos1_tile in ["-", "L", "F"]:
        pos1 = (pos1[0], pos1[1]+1)
        not_changing = 3
    else:
        break
    if not_changing == 1:
        h = haut
        haut = changes[input2[pos1[0]][pos1[1]]]["gauche"][gauche]
        if haut is None:
            haut = h
        print(f"{pos1_tile} => {input2[pos1[0]][pos1[1]]}, ({gauche = }, {haut = }), 1")
    else:
        g = gauche
        gauche = changes[input2[pos1[0]][pos1[1]]]["haut"][haut]
        if gauche is None:
            gauche = g
        print(f"{pos1_tile} => {input2[pos1[0]][pos1[1]]}, ({gauche = }, {haut = }), 2")

    pos1_tile = input2[pos1[0]][pos1[1]]
    input2[pos1[0]][pos1[1]] = n_etapes
for i, line in enumerate(input2):
    for j, elem in enumerate(line):
        if elem == "*":
            input3[i][j] = "*"
